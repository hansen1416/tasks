        ...
        # If both endpoints are colored
            ...
            if nodes_color[edge[0]] == nodes_color[edge[1]]:
                ...
                if len(color_nodes[nodes_color[edge[0]]]) > len(color_nodes[nodes_color[edge[1]]]):
                    ...
                    deleted_color_nodes = color_nodes.pop(
                        nodes_color[edge[1]], None)
...
def tsp(points):
    ...
    adjacency_list_sorted = {k: v for k, v in sorted(
        adjacency_list.items(), key=lambda item: item[1])}
    ...
    odd_nodes_admatrix = [
        [0 for _ in range(ston_len)] for _ in range(ston_len)]
        ...
        for i2, p2 in enumerate(spanning_tree_odd_nodes):
            if p1 != p2:
                ...
                if p1[0] < p2[0] or (p1[0] == p2[0] and p1[1] < p2[1]):
                    odd_nodes_admatrix[i1][i2] = adjacency_list_sorted[(
                        p1, p2)]
                    odd_nodes_admatrix[i2][i1] = adjacency_list_sorted[(
                        p1, p2)]
    ...
    for i in range(ston_len):
        for j in range(i+1, ston_len):
            weighted_edges.append(
                (i, j, edge_max_weight - odd_nodes_admatrix[i][j]))

    # print(weighted_edges)

    G.add_weighted_edges_from(weighted_edges)

    perfect_matching = nx.algorithms.matching.max_weight_matching(
